	...
	public int signInRun(int flag,String user_name,String user_password) throws SQLException, ClassNotFoundException{
		...
		while(rs.next()) {
			...
			if(name.equals(user_name)) {
				...
				if(!password.equals(String.valueOf(user_password))) {
					flag = 2;
				}
			}
		}
...
//		ps.setInt(6, number);
	...
	public void createUserTable(String name,String password) throws SQLException {
		...
		String sqlStr2 = "CREATE TABLE " + name + "(number INT NOT NULL PRIMARY KEY AUTO_INCREMENT,kind CHAR(1) CHECK(种类='猫' OR 种类='狗'),varieties VARCHAR(7) NOT NULL,unit INT DEFAULT 0 NOT NULL,total DECIMAL(7,2) NOT NULL)AUTO_INCREMENT=1;";
		PreparedStatement ps2 = conn.prepareStatement(sqlStr2);
		ps2.executeUpdate();
		ps2.close();
		JDBCUtils.close(conn);
